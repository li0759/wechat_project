...
Component({
  ...
  data: {
    ...
    // 会长选择通讯录导航视图（pab = president addressbook）
    pabDeptTree: [],
    pabDeptExpand: {},
    pabNavStack: [],
    pabNavTitle: '',
    ...
    // isotope 头像组- 成员（与 club-manage-panel 添加成员弹窗一致）
    memberAvatarItems: [],
    memberImageStyle: { borderRadius: '50%' },
    memberAvatarIsoHeight: '150rpx',
    roleDisplayMap: {
      all: '全部',
      member: '会员',
      director: '理事',
    ...
    try {
      ...
      if (res.Flag == 4000) {
        ...
        const resultsWithStatus = res.data.users.map(user => {
          const isInMembers = this.data.membersList.some(member => member.user_id === user.user_id)
          const isPresident = this.data.selectedPresident && this.data.selectedPresident.id === user.user_id
          return {
            ...user,
            isExistingMember: isInMembers || !!isPresident,
            isPresident: !!isPresident,
            isPending: false,
            is_current_user: !!user.is_current_user,
            member_id: user.member_id || null,
            role: user.role || null,
            role_display: user.role_display || null
          }
        });
  ...
  setPresident(user) {
    ...
    const updatedMembersList = this.data.membersList.filter(member =>
      ...
      // 从成员isotope 中移除该成员
    const memberIso = this.selectComponent('#memberAvatarIsotope')
    ...
    if (this.data.searchResults && this.data.searchResults.length > 0) {
      const updatedSearchResults = this.data.searchResults.map(searchUser => {
        ...
        return {
          ...
          isExistingMember: isInMembers || !!isPresident,
          isPresident: isPresident,
          isPending: !!searchUser.isPending,
          member_id: searchUser.member_id || null,
        };
      });
  ...
  removeMember(e) {
    ...
    if (this.data.selectedPresident && String(this.data.selectedPresident.id) === String(userId)) {
      wx.showToast({ title: '该用户为会长，请在上方会长入口更换', icon: 'none' })
      return
    }
  ...
  /** 以下与 club-manage-panel 添加成员 WXML 对齐；创建协会场景无审批/角色变更能力 */
  onSearchUserApprovalOpinionChange(e) {
    this.setData({ searchUserApprovalOpinion: e.detail.value || '' })
  },
  changeRole() {
    wx.showToast({ title: '创建协会时请在提交后于协会管理中变更角色', icon: 'none' })
  },
  approveSearchUserApplication() {
    wx.showToast({ title: '创建协会无需审批入会', icon: 'none' })
  },
  rejectSearchUserApplication() {
    wx.showToast({ title: '创建协会无需审批入会', icon: 'none' })
  },
  quickApproveApplication() {
    wx.showToast({ title: '创建协会无需审批入会', icon: 'none' })
  },
  quickRejectApplication() {
    wx.showToast({ title: '创建协会无需审批入会', icon: 'none' })
  },
