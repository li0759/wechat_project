...
Component({
  ...
  lifetimes: {
    attached() {
      ...
      const timeStr = this.data.scheduleForm.activity_time || this.data.timeOfDayValue || '09:00';
  ...
  onScheduleExpand() {
    ...
    onTimeSelectorExpand() {
      ...
      const h = String(parseInt(parts[0] || '9', 10) || 0).padStart(2, '0');
      const m = String(parseInt(parts[1] || '0', 10) || 0).padStart(2, '0');
      const hIdx = Math.max(0, this.data.timeHours.indexOf(h));
      const mIdx = Math.max(0, this.data.timeMinutes.indexOf(m));
      this.setData({ timeOfDayValue: currentTime, timePickerValue: [hIdx, mIdx] });
    },
    ...
    onTimePickerChange(e) {
      ...
      const h = this.data.timeHours?.[val[0]] ?? '09';
      const m = this.data.timeMinutes?.[val[1]] ?? '00';
      const time = `${h}:${m}`;
      this.setData({ timePickerValue: val });
      this.onTimeInput({ detail: time });
    },

    onAdvanceHoursChange(event) {
      const value = event?.detail?.value ?? event?.detail ?? 1;
