...
def sync_wecom_users_to_db():
    ...
    try:
        ...
        # 3. 遍历所有部门，获取用户列表（全量一致：记录本次出现的 wecomUserID）
        ...
        for dept in departments:
            ...
            for user_info in dept_users:
                ...
                if not wecom_id:
                    continue
                seen_wecom_ids.add(wecom_id)
                # 检查用户是否已存在（通过wecomUserID）
                existing_user = User.query.filter_by(wecomUserID=wecom_id).first()
                ...
                user_data = {
                    ...
                    'department': ','.join([n for n in department_names if n]) if department_names else '',
                    'departmentID': primary_dept_id,
                    'position': norm_str(user_info.get('position', '')).strip(),
                ...
                if existing_user:
                    ...
                    print(f"新增用户: {user_info.get('name')} ({wecom_id})")

        # 4. 全量一致：软删除本次未出现的通讯录用户
        try:
            if seen_wecom_ids:
                missing_users = User.query.filter(
                    User.wecomUserID.isnot(None),
                    User.wecomUserID != '',
                    User.wecomUserID.notin_(list(seen_wecom_ids)),
                    User.isDelete == False
                ).all()
                for u in missing_users:
                    u.isDelete = True
                if missing_users:
                    print(f"同步软删除用户数: {len(missing_users)}")
        except Exception as e:
            print(f"同步软删除处理失败: {e}")

        # 提交所有更改
        db.session.commit()
        print(f"用户同步完成: 更新{sync_count['updated']}个, 新增{sync_count['inserted']}个, seen={len(seen_wecom_ids)}")
        return True
