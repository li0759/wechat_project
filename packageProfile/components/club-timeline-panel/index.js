...
Component({
  ...
  data: {
    ...
    // 对齐 event-create-panel：内嵌 t-date-time-picker 编辑态
    timeEdit: {
      clubfeeDateTime: '',
      payRangeStart: new Date(new Date().setDate(1)).toISOString().split('T')[0],
      payRangeEnd: new Date().toISOString().split('T')[0]
    },
    paymentFeeList: [],
    ...
    // 旧版弹窗字段（避免 vant-field 收到 null/undefined 报警）
    expenseModalVisible: false,
    incomeModalVisible: false,
    newExpense_feement: '',
    newExpense_description: '',
    newExpense_date: '',
    newExpense_time: '',
    newIncome_total_fee: '',
    newIncome_description: '',
    newIncome_date: '',
    newIncome_time: '',
  },
  ...
  methods: {
    ...
    // ========= 对齐 event-create-panel：picker 值归一化 =========
    ...
    normalizeDatePickerValue(raw) {
      const v = this.normalizePickerValue(raw)
      if (!v) return ''
      return String(v).slice(0, 10)
    },
    parseLocalDateTimeToISO(display) {
      if (!display) return ''
      const s = String(display).trim().replace(' ', 'T')
      const d = new Date(s)
   ...
   * 显示创建支出弹窗
   */
  navigateToCreateClubfee: function(e) {
    // 初始化支出表单数据
    const now = new Date();
    ...
    this.setData({
      ...
      this.setData({ 'timeEdit.clubfeeDateTime': formattedDate })
      const t = e?.changedTouches?.[0] || e?.touches?.[0]
      const sys = wx.getSystemInfoSync()
      const x = t?.clientX ?? (sys.windowWidth / 2)
      const y = t?.clientY ?? (sys.windowHeight / 2)
      const fs = this.selectComponent('#clubfeeCreateFs')
      if (fs && typeof fs.expand === 'function') fs.expand(x, y)
    });
  },
   ...
   * 显示创建收款弹窗
   */
  navigateToCreatePayment: function(e) {
    // 初始化收款表单数据
    const now = new Date();
    ...
    this.setData({
      ...
      this.setData({
        'timeEdit.payRangeStart': this.data.paymentSelectedDateRange?.[0] || this.data.timeEdit.payRangeStart,
        'timeEdit.payRangeEnd': this.data.paymentSelectedDateRange?.[1] || this.data.timeEdit.payRangeEnd
      })
      const t = e?.changedTouches?.[0] || e?.touches?.[0]
      const sys = wx.getSystemInfoSync()
      const x = t?.clientX ?? (sys.windowWidth / 2)
      const y = t?.clientY ?? (sys.windowHeight / 2)
      const fs = this.selectComponent('#paymentCreateFs')
      if (fs && typeof fs.expand === 'function') fs.expand(x, y)
    });
  },
  ...
  showMonthPicker: function() {
    ...
    const baseYear = (this.data.activeTab === 0 && this.data.listDisplayYear) ? this.data.listDisplayYear : this.data.currentDisplayYear;
    const baseMonth = (this.data.activeTab === 0 && this.data.listDisplayMonth) ? this.data.listDisplayMonth : this.data.currentDisplayMonth;
    const yearIdx = Math.max(0, years.indexOf(baseYear));
    const monthIdx = Math.max(0, (baseMonth - 1));
    this.setData({
  ...
  closeCreateClubfeePopup: function() {
    const fs = this.selectComponent('#clubfeeCreateFs')
    if (fs && typeof fs.collapse === 'function') fs.collapse()
    this.setData({ showClubfeeDatetimePicker: false });
  },
  ...
  // 支出时间已改为自研小弹窗（t-date-time-picker 内嵌），不再使用 vant datetime-picker

  // ========= 支出时间（内嵌 t-date-time-picker，对齐 event-create-panel） =========
  onClubfeeTimePick(e) {
    const val = this.normalizePickerValue(e?.detail?.value)
    this.setData({ 'timeEdit.clubfeeDateTime': val })
  },
  onClubfeeTimeChange(e) {
    const val = this.normalizePickerValue(e?.detail?.value)
    this.setData({ 'timeEdit.clubfeeDateTime': val })
  },
  onClubfeeTimeConfirm(e) {
    const val = this.normalizePickerValue(e?.detail?.value)
    this.setData({ 'timeEdit.clubfeeDateTime': val })
  },
  confirmClubfeeTime() {
    const v = (this.data.timeEdit && this.data.timeEdit.clubfeeDateTime) || ''
    if (!v) return
    const iso = this.parseLocalDateTimeToISO(v)
    if (!iso) return
    this.setData({
      'clubfeeForm.createDate': iso,
      clubfeeFormattedDate: v,
      clubfeeCurrentDate: new Date(iso).getTime()
    }, () => {
      this.selectComponent('#ct-datetime')?.collapse?.()
    })
  },

  onClubfeeDateTimeCancel: function() { return },
  ...
  closeCreatePaymentPopup: function() {
    const fs = this.selectComponent('#paymentCreateFs')
    if (fs && typeof fs.collapse === 'function') fs.collapse()
    this.setData({ paymentCalendarShow: false });
  },
  ...
  onPayRangeStartPick(e) {
    const v = this.normalizeDatePickerValue(e?.detail?.value)
    if (!v) return
    this.setData({ 'timeEdit.payRangeStart': v })
  },
  onPayRangeStartChange(e) {
    const v = this.normalizeDatePickerValue(e?.detail?.value)
    if (!v) return
    this.setData({ 'timeEdit.payRangeStart': v })
  },
  onPayRangeStartConfirm(e) {
    const v = this.normalizeDatePickerValue(e?.detail?.value)
    if (!v) return
    this.setData({ 'timeEdit.payRangeStart': v })
  },
  onPayRangeEndPick(e) {
    const v = this.normalizeDatePickerValue(e?.detail?.value)
    if (!v) return
    this.setData({ 'timeEdit.payRangeEnd': v })
  },
  onPayRangeEndChange(e) {
    const v = this.normalizeDatePickerValue(e?.detail?.value)
    if (!v) return
    this.setData({ 'timeEdit.payRangeEnd': v })
  },
  onPayRangeEndConfirm(e) {
    const v = this.normalizeDatePickerValue(e?.detail?.value)
    if (!v) return
    this.setData({ 'timeEdit.payRangeEnd': v })
  },
  ...
  confirmPaymentRange() {
    const start = this.data.timeEdit.payRangeStart
    const end = this.data.timeEdit.payRangeEnd
    if (!start || !end) return
    ...
    this.setData({ paymentSelectedDateRange: [start, end] }, () => {
      this.fetchPaymentFeeList()
      this.selectComponent('#ct-pay-range')?.collapse?.()
    })
  },
