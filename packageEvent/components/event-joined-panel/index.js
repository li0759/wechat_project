...
Component({
  ...
  async processEventData(eventData) {
    ...
    // 拉取活动成员列表，用于展示最近参加/最近打卡头像（最多取6个用于“更多”暗示）
    this.loadRecentMembers().catch(() => {});
  ...
  loadRecentMembers() {
    return new Promise((resolve, reject) => {
      ...
      wx.request({
        url: app.globalData.request_url + `/event/${this.data.eventId}/members`,
        method: 'GET',
        header: {
        ...
        success: (res) => {
          if (res.data && res.data.Flag == 4000) {
            ...
            const byJoin = [...(Array.isArray(members) ? members : [])]
              .filter(m => m && m.join_date)
              .sort((a, b) => new Date(b.join_date).getTime() - new Date(a.join_date).getTime())
              .slice(0, 6);

            const byClockin = [...(Array.isArray(members) ? members : [])]
              .filter(m => m && m.clockin_date)
              .sort((a, b) => new Date(b.clockin_date).getTime() - new Date(a.clockin_date).getTime())
              .slice(0, 6);

            this.setData({
              recentJoinMembers: byJoin,
              recentClockinMembers: byClockin
            });
            resolve();
          } else {
            reject(new Error(res.data?.message || '获取活动成员失败'));
          }
        },
        fail: reject
      });
  ...
  // 加载活动动态列表（显示所有参与者发布的动态；仅本人动态可删除）
  async loadEventMoments(page = 1) {
    ...
    try {
      ...
      if (response.data && response.data.Flag == 4000) {
        ...
        const realData = moments.map(moment => ({
          ...moment,
